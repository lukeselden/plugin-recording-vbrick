...
async function startRecording(
  title: string,
  timeoutSeconds = 60
): Promise<ApiResult<Partial<Recording>>> {
  ...
  const response = await makeApiRequest<Record<string, string>>(url, body, {
    method: 'POST',
    timeoutSeconds
  })
...
async function stopRecording({
  rtmpStreamKey
}: Recording): Promise<ApiResult<void>> {
  // ignore request if no active recording
  if (rtmpStreamKey == null || rtmpStreamKey === '') {
