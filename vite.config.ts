...
export function updateDevConfig(cfg: BuildConfig = config): BuildConfig {
  ...
  if (!infinityUrl && URL.canParse(redirectUri) && !redirectUri.includes('pexip.example.com')) {
    console.warn(`No infinity_url set in vite.json, using configured vbrick redirect URL ${redirectUri}`);
  ...
  if (!infinityUrl && sipDomain && sipDomain !== "pexip.example.com") {
    console.warn(`No infinity_url set in vite.json, using infinity.sip_domain (${sipDomain})`);
    cfg.infinity_url = infinityUrl = `https://${sipDomain}`;
  }
  if (!infinityUrl) {
    throw new Error('FATAL No pexip url (infinity_url) specified in vite.json. Check README.md for instructions')
  }
  return cfg;
}
...
const overrideConfigPlugin = (): Plugin => ({
  ...
  configureServer(server) {
    ...
    async function overrideManifest(): Promise<{ ok: true, manifest: any } | { ok: false, statusCode: number, statusMessage: string }> {
      try {
