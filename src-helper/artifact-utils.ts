...
export function pathJoin(...paths: string[]): string {
  return paths
    ...
    .reduce(
      (agg, path) =>
        `${agg !== '' ? agg.replace(/\/*$/, '/') : ''}${path.replaceAll(/[/\\]+/g, '/')}`,
      ''
    )
}
...
export function encodeArtifacts(input: Artifacts): Artifacts<string> {
  return {
    ...
    assets: Object.fromEntries(
      Object.entries(input.assets).map(([key, value]) => [
        key,
        typeof value === 'string' ? value : toBase64(value)
      ])
    )
  }
}
...
export function decodeArtifacts(
  encoded: Artifacts<string>
): Artifacts<Uint8Array> {
  return {
    ...encoded,
    assets: Object.fromEntries(
      Object.entries(encoded.assets).map(([key, value]) => [
        key,
...
export function addArtifactsToZip(
  contents: Unzipped,
  listing: Artifacts<Uint8Array>,
  directory = ''
): Unzipped {
